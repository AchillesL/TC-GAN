...
class CSWinBlock(nn.Module):
    ...
    def __init__(self, dim, reso, num_heads,
                 split_size=7, mlp_ratio=2., qkv_bias=False, qk_scale=None,
...
if __name__ == '__main__':
    # model = CSWinBlock(dim=64, reso=128, num_heads=32, split_size=8, drop=0.3, drop_path=0.3)

    model = CSWin_64_24322_small_224()
    data = torch.rand((5, 3, 224, 224))
    # data = rearrange(data, 'b c h w -> b (h w) c')
    data = model(data)
    # data = rearrange(data, 'b (h w) c -> b c h w', h=128)
